...
format: html
editor: visual
output-file: results/04_describe_data.html
---
...
Counting the sample size for each group (diabetic and none-diabetic) based on the col names starting with "GSM".
...
The mean, median, and standard error for each gene are displayed below:
...
gene_medians_none_T2D <- apply(samples_T2D, 1, median, na.rm = TRUE)
gene_sd_none_T2D <- apply(samples_T2D, 1, sd, na.rm = TRUE)
...
## Visualization of the mean gene expression

A histogram for each group shows the mean gene expression of the raw data.
