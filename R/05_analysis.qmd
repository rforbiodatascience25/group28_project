...
df_non_T2D <- read.csv("../data/03_control_data.csv")
```

### Gene expression differences

For each dataset, we compute the median gene expression across samples (columns starting with "GSM"). We also compute the standard deviation as a measure of variability. We then keep only the gene identifier, median expression, and SD.

```{r}
# For type 2 diabetes:
df_T2D_medians <- df_T2D |>
  mutate(
    median_expression_T2D = rowMedians(as.matrix(across(starts_with("GSM")))),
    sd_expression_T2D = rowSds(as.matrix(across(starts_with("GSM"))))
  ) |>
  select(
    IDENTIFIER, median_expression_T2D, sd_expression_T2D
  )

# For control group:
df_control_medians <- df_non_T2D |>
  mutate(
    median_expression_control = rowMedians(as.matrix(across(starts_with("GSM")))),
    sd_expression_control = rowSds(as.matrix(across(starts_with("GSM"))))
  ) |>
...
These genes will be used as a reduced data set for follow-up analysis later.
...
A positive value means the gene is more expressed in T2D; a negative value means it is lower.

```{r}
#| label: Expression_median_plot
...
ggplot(
  top_30_plot,
  aes(x = reorder(gene_label, median_difference), y = median_difference, fill = direction)
) +
  geom_col() +
  coord_flip() +
  labs(
...
## Boxplot of median expressions

To illustrate the underlying sample-level expression, we show boxplots of the top 10 genes with the largest median differences.

```{r}
#| label: Expression_median_boxplot
...
ggplot(plot_data, aes(x = IDENTIFIER, y = expression, fill = group)) +
  geom_boxplot() +
  labs(
    title = "Expression Distribution of Top 10 Differential Genes",
    x = "Gene", 
    y = "Expression Level") +
  theme_minimal() +
  theme(
